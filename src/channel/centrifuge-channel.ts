...
export class CentrifugeChannel extends Channel {
            ...
            function (ctx) {
                return new Promise((resolve, reject) => {
                    fetch(options.authEndpoint, {
                        method: 'POST',
                        headers: new Headers({ 'Content-Type': 'application/json', ...options.auth.headers }),
                        body: JSON.stringify(ctx)
                    })
                    .then(function (rsp) {
                        if (! rsp.ok) {
                            throw new Error(`Unexpected status code ${rsp.status}`);
                        }
                        return rsp.json();
                    })
                    .catch(function (err) {
    ...
    subscribe(): void {
        ...
        this.subscription = this.centrifuge.newSubscription(this.name, this.options.public ? {} : {
